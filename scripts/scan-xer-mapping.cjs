...
function parseSchemaMaps(tsPath) {
  ...
  for (let raw of lines) {
    const line = raw.trim();
    // detect table name
    const tmatch = line.match(/^table:\s*\[\s*'([^']+)'\s*,\s*'[^']+'\s*\],?$/);
    if (tmatch) {
      currentTable = tmatch[1];
      if (!map.has(currentTable)) map.set(currentTable, new Set());
      inColumns = false;
      continue;
    }
    if (!currentTable) continue;
    // detect start of columns
    if (line.startsWith('columns:')) {
      inColumns = true;
      continue;
    }
    if (!inColumns) continue;
    // detect end of columns array
    if (line.startsWith(']')) {
      inColumns = false;
      continue;
    }
    // parse a column tuple like ['col','prop','TYPE'] with optional whitespace
    const cmatch = line.match(/^\[\s*'([^']+)'\s*,\s*'[^']+'\s*,\s*'(?:TEXT|OTEXT|NUMBER|ONUMBER|DATE|ODATE)'\s*\],?$/);
    if (cmatch) {
      const col = cmatch[1];
      map.get(currentTable).add(col);
    }
  }
  return map;
}
