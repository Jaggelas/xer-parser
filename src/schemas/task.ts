...
export class Task {
	...
	constructor(_xer: XER, header: string[], row: string[]) {
		...
		this.autoComputeAct = row[header.indexOf('auto_compute_act_flag')] === 'Y';
		...
		this.totalFloatHrCnt = Number(row[header.indexOf('total_float_hr_cnt')]);
		this.freeFloatHrCnt = Number(row[header.indexOf('free_float_hr_cnt')]);
		this.remainDrtnHrCnt = Number(row[header.indexOf('remain_drtn_hr_cnt')]);
		...
		this.targetDrtnHrCnt = Number(row[header.indexOf('target_drtn_hr_cnt')]);
		...
		this.expectEndDate = optionalDate(row[header.indexOf('expect_end_date')]);
		...
		this.floatPathOrder = optionalNumber(row[header.indexOf('float_path_order')]);
		...
		this.actThisPerWorkQty = Number(row[header.indexOf('act_this_per_work_qty')]);
		this.actThisPerEquipQty = Number(row[header.indexOf('act_this_per_equip_qty')]);
		this.externalEarlyStartDate = optionalDate(row[header.indexOf('external_early_start_date')]);
		this.externalLateEndDate = optionalDate(row[header.indexOf('external_late_end_date')]);
	...
	public get taskPredecessors(): TaskPredecessor[] {
		return this.xer.taskPredecessors.filter((tp) => tp.taskId === this.taskId);
	}
	...
	public get TaskMemos(): TaskMemo[] {
		return this.xer.taskMemos.filter((taskMemo) => taskMemo.taskId === this.taskId);
	}
	...
	public get taskActivityCodes(): TaskActivityCode[] {
		return this.xer.taskActivityCodes.filter((taskActivityCode) => taskActivityCode.taskId === this.taskId);
	}
}
